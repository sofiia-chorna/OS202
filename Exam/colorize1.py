...
def create_field(values, ifield, nb_layers, prolong_field=False):
    ...
    # À partir d'un tableau contenant n champs de taille ny x nx, on
...
def assembly_row(
        image_size, i_start, intensity, means, variance, pos, pt_rows, ind_cols, coefs
):
    """
...
def do_local_calculation(local_width):
    ...
    cropped_marked_image = im_marked.crop((offset, 0, offset + fragment_width, height - 1))

    # Apply colorisation of the fragment
    return colorize(cropped_image, cropped_marked_image)
...
if __name__ == '__main__':
    ...
    # Get starting time of calculation
    start_time = time.time()

    # Separate the image by fragments based on the number of processes
    fragment = get_fragment_width(width, size)
    offset = fragment * rank
    local_width = width / size

    colorized_image = do_local_calculation(local_width)

    # Collect all fragments
    gathered_image_fragments = comm.gather(colorized_image, root=0)
    process_calculation_times = comm.gather(time.time() - start_time, root=0)

    if rank == 0:
        print("Calculation time by each process: ", process_calculation_times)
        print("Total calculation time: ", np.max(process_calculation_times))

        x_offset = 0
        result_image = Image.new("RGB", (width, height))

        # Iterate in the fragments and join them all together in one
        for image in gathered_image_fragments:
            result_image.paste(image, (x_offset, 0))
            x_offset += image.size[0]

        # Save image
        result_image.save(OUTPUT_FILENAME, 'PNG')
